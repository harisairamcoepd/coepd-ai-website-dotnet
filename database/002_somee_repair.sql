...
IF EXISTS (SELECT 1 FROM dbo.staff WHERE role IS NULL) UPDATE dbo.staff SET role = 'staff' WHERE role IS NULL;
IF EXISTS (SELECT 1 FROM dbo.staff WHERE status IS NULL) UPDATE dbo.staff SET status = 'active' WHERE status IS NULL;
IF EXISTS (SELECT 1 FROM dbo.staff WHERE created_at IS NULL) UPDATE dbo.staff SET created_at = SYSUTCDATETIME() WHERE created_at IS NULL;

IF EXISTS (SELECT 1 FROM dbo.leads WHERE source IS NULL) UPDATE dbo.leads SET source = 'webpage' WHERE source IS NULL;
IF EXISTS (SELECT 1 FROM dbo.leads WHERE created_at IS NULL) UPDATE dbo.leads SET created_at = SYSUTCDATETIME() WHERE created_at IS NULL;
